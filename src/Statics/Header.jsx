    ...
    return (
        <div className="flex justify-between  items-center py-4 px-4 font-bold text-base md:text-xl top-0 left-0 z-50 h-[70px] gap-2 max-lg:px-2 max-lg:text-sm">
            ...
            <div className="flex justify-between gap-6 items-center cursor-pointer ">
                <Menu className="font-bold text-xl hidden max-md:block md:hidden h-10 w-10 m-4 cursor-pointer z-50 relative" onClick={() => setOpenMenu(!openMenu)} />

                <div
                    className={`fixed top-0 left-0 w-[70%] max-w-[300px] h-screen flex flex-col items-start  gap-4 font-semibold text-lg  z-40 transition-all duration-300 ease-in-out ${openMenu ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0"
                        }`}
                    style={{ backgroundColor: "rgba(0, 0, 0, 0.2)", boxShadow: "4px 0 10px rgba(0,0,0,0.3)" }}
                >
            ...
            <div className="flex justify-between gap-6 cursor-pointer ">
                ...
                <div className="flex gap-5 cursor-pointer max-md:hidden">
                    <button className="bg-white text-black hover:bg-[#800000] hover:text-white font-bold py-2 px-4 rounded">Sign Up</button>
                    <button className="bg-white text-black hover:bg-[#800000] hover:text-white font-bold py-2 px-4 rounded">Log in</button>
                </div>
            </div>
        </div>
    )
